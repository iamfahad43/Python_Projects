...
Template_DF = pd.DataFrame(columns=["TITLE", "PRICE", "CATEGORIES", "DESCRIPTION", "IMAGE_LINK"])

for base_url, scrap_url in enumerate(url_list):
    url = scrap_url
    ...
    for index, products in enumerate(product_list[::2]):
        ...
        #print(f"Hold on Ahsan!, I'm scraping the products\n")
        
        Title = new_soup.find("h1", {"class": "title"}).text.strip('\r\n \t.?')
        Price = new_soup.find("h2", {"class": "price"}).text.strip('\r\n \t.?')
        Price = Price[2:7]
        
        Image = None
        
        images = new_soup.findAll('img')
        for image in images[1:2]:
            #print image source
            image_link = "https:" + image['src'] 
            Image = image_link
            
        Category = new_soup.findAll("span", {"itemprop": "title"})[1].text.strip('\r\n \t.?')
        ...
        Template_DF = Template_DF.append({"TITLE":Title, "PRICE":Price, "CATEGORIES":Category, 
                                 "DESCRIPTION":Description, "IMAGE_LINK":Image}
                                ,ignore_index=True)

Template_DF.to_csv('Six.csv', sep='\t', encoding='utf-8')
Template_DF.to_excel("Six.xlsx")
